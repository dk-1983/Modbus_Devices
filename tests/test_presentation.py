...
@pytest.mark.asyncio
async def test_c2000r_st_uses_dedicated_native_glass_break_card(registry_hass):
    device = Device("rst", "rst-stable", model="С2000Р-СТ исп.01")
    result = await build(
        registry_hass,
        device,
        [
    ...
    assert result.profile_id == "bolid_c2000r_st_01"
    assert result.card["type"] == "entities"
    assert entity_ids(result) == [
        "sensor.rst_glass_break_state",
        "binary_sensor.rst_glass_break",
        "binary_sensor.rst_enclosure_tamper",
        "sensor.rst_main_battery_state",
    ]
...
@pytest.mark.asyncio
async def test_wired_smk_card_contains_only_raw_opening_state(registry_hass):
    device = Device("smk", "smk-stable", model="С2000-СМК исп.04")
    result = await build(
        registry_hass,
        device,
...
@pytest.mark.asyncio
async def test_radio_smk_card_includes_configured_external_input_before_raw_opening(
    registry_hass,
):
    device = Device("rsmk", "rsmk-stable", model="С2000Р-СМК")
    result = await build(
