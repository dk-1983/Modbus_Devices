...
- [Установка](#установка)
- [Добавление оборудования](#добавление-оборудования)
- [Поддерживаемые подключения](#поддерживаемые-подключения)
- [Поддерживаемое оборудование](#поддерживаемое-оборудование)
- [Устранение неполадок](#устранение-неполадок)
- [История релизов](https://github.com/dk-1983/Modbus_Devices/releases)

Изменения отдельных версий описаны на странице GitHub Releases. В этом README
находятся актуальные сведения об интеграции, оборудовании и его настройке.
...
### [Bolid](https://bolid.ru/) — прямое подключение и оборудование Orion
...
| [Samsung-ESP32-Modbus](https://github.com/dk-1983/Samsung-ESP32-MQTT-Modbus) | Modbus RTU или Modbus TCP | Управление климатом через Samsung UART, полные режимы вентилятора/качания/preset, подтверждаемые записи и диагностика связи/команд |
...
Samsung-ESP32-Modbus — каталожное имя контроллера
[Samsung-ESP32-MQTT-Modbus](https://github.com/dk-1983/Samsung-ESP32-MQTT-Modbus)
в Modbus Devices. MQTT для работы интеграции не требуется. Профиль использует
собственную zero-based карту регистров проекта и не связан с заводским профилем
Samsung MIM-B19N(T). Контроллер сам поддерживает соединение Samsung UART;
подключённый кондиционер не является Modbus slave.
...
| Модель | Подключение | Основные возможности |
|---|---|---|
| ER-G-220-05 | Прямой Modbus | Мониторинг и состояния насосного преобразователя; секундный опрос рабочих данных; команды пуска, останова и аварий; управление Y1/Y2; защищённая настройка P001…P137 и расписаний реле времени |
...
Параметры конфигурации сохраняют документированные обозначения `Pxxx` и
используют масштабы протокола и актуальные зависимые ограничения. Они
обновляются отдельно от рабочих данных двумя групповыми чтениями FC03 каждые
30 секунд. Записи сериализуются, требуют корректного зеркального ответа FC06 и
точного перечитывания FC03 и публикуются только после подтверждения. Изменение
P006 обновляет допустимые диапазоны без перезаписи зависимых значений. Если
P102 уменьшается ниже P101, сначала уменьшается и подтверждается P101.

P132/P135 представлены штатными сущностями времени, а P134/P137 — отдельными
переключателями дней недели для обоих каналов реле времени. P122 (адрес прибора)
и P123 (скорость) намеренно недоступны для записи: измените их на приборе и
пересоздайте запись интеграции. Для P127/P128, относящихся к часам
преобразователя, требуется отдельное управление RTC и аппаратная проверка.
Зарезервированные регистры никогда не записываются.

Секундный опрос улучшает наблюдение, но не является журналом событий: короткий
переход между запросами может быть пропущен. Для контроля безопасности следует
использовать сохраняемый код аварии и собственную диагностику преобразователя.
Чтобы журналировать подтверждённые записи ERMAN FC05/FC06 без подробного
логирования остальных компонентов, добавьте:
