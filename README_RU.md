...
## В разработке — расписание реле времени ERMAN

Для реле времени ER-G-220-05 параметры P132/P135 представлены штатными
сущностями времени Home Assistant, а P134/P137 — отдельными переключателями
понедельника–воскресенья для каждого канала. Время записывается в
документированном десятичном формате `ЧЧ.ММ` с минутной точностью. При изменении
дня интеграция последовательно читает регистр по FC03, меняет только его
документированный бит, записывает полную маску по FC06 и проверяет её точным
перечитыванием FC03, сохраняя все остальные биты.

Аудит охватывает все документированные настройки P001–P137. P122 (адрес
прибора) и P123 (скорость обмена) намеренно недоступны, поскольку их изменение
может оборвать действующее соединение. P127/P128 относятся к часам самого
прибора, а не к расписанию реле, и оставлены до отдельной реализации RTC и
физической проверки. Все остальные незарезервированные настройки представлены.

## Новое в 1.7.1 — исправление идентификации ERMAN

Все числовые параметры настройки ER-G-220-05 теперь относятся к тому же
устройству Home Assistant, что рабочие датчики и остальные элементы управления.
Лишнее второе устройство «Variable-frequency drive», появившееся в 1.6.0,
больше не создаётся. Названия числовых и перечислимых параметров теперь также
выбираются по языку Home Assistant и больше не закреплены на английском.

## Новое в 1.7.0 — Samsung MIM-B19N(T)

Для Samsung MIM-B19N и MIM-B19NT добавлен единый профиль **MIM-B19N(T)** в
разделе `Кондиционирование и охлаждение → Samsung`. Профиль использует
стандартную карту регистров из общей инструкции Samsung, управляет питанием,
режимом, заданной температурой, вентилятором и вертикальным потоком, а также
показывает текущую температуру, связь и коды ошибок шлюза и устройства.

При настройке отдельно задаются Modbus-адрес платы MIM и адрес устройства
Samsung за ней (0–47). Модель подключённого кондиционера вводить не требуется:
MIM сам поддерживает соединение R1/R2 и представляет устройства в Modbus.
Реализация покрыта протокольными тестами; проверка на физической плате пока не
выполнена.

## Новое в 1.6.0 — управление настройками ERMAN

Для ERMAN ER-G-220-05 добавлены 25 числовых и 12 перечислимых параметров
настройки со страниц 8–9 протокола. В названиях сохранены документированные
обозначения `Pxxx`, значения используют штатные масштабы и пределы, а зависимые
ограничения учитывают фактические значения P006/P102.

Настройки обновляются двумя групповыми FC03-запросами раз в 30 секунд отдельно
от секундного опроса рабочих данных. Каждое изменение выполняется
сериализованной командой FC06, проходит проверку зеркального ответа и точное
перечитывание FC03 до публикации нового значения в Home Assistant.
Зарезервированные регистры никогда не записываются. P122 (адрес прибора) и P123
(скорость) намеренно недоступны: их следует изменить на самом приборе и
пересоздать подключение, чтобы запись не оборвала действующую связь.

Изменение P006 меняет только допустимые диапазоны зависимых параметров давления;
сама интеграция никогда не перезаписывает их значения. Если P102 уменьшается
ниже фактического P101, интеграция сначала безопасно уменьшает и подтверждает
P101, а затем записывает P102. Каждая запись ERMAN по FC05/FC06 фиксируется в журнале Home
Assistant с адресом прибора, функцией, регистром, параметром или командой,
необработанным значением и подтверждением; реквизиты подключения не журналируются.

Чтобы собрать этот журнал, не включая подробную диагностику остальных
компонентов, добавьте в `configuration.yaml` и перезапустите Home Assistant:
...
## Новое в 1.5.1 — ускоренный мониторинг ERMAN

Блок рабочих данных ERMAN ER-G-220-05 теперь опрашивается один раз в секунду
вместо общего интервала интеграции в пять секунд. Это повышает детализацию
состояния ПЧ, выходной частоты, текущего давления и тока двигателя, не изменяя
частоту опроса остальных приборов. Интервал закреплён за моделью; пользователь
не может установить небезопасный субсекундный опрос.

ER-G-220-05 является ведомым Modbus-устройством и не может самостоятельно
инициировать передачу нового состояния. Home Assistant в роли мастера получает
только значения регистров, существующие в момент очередного опроса. Поэтому
опрос не является журналом событий: изменение, которое началось и завершилось
между двумя секундными запросами, может быть пропущено. Для аварий следует
использовать сохраняемый код ошибки и собственную диагностику преобразователя,
а не только его мгновенное состояние в Home Assistant.
...
- Явно описанные модели оборудования 4VRS, APC, Bolid, Daikin, Dyna Drive,
  ERMAN, Haier, Owen, Samsung и Zuked.
...
| Модель | Подключение | Основные возможности |
|---|---|---|
| MIM-B19N(T) | Modbus RTU, включая RTU over TCP через прозрачный шлюз | Питание, режим HVAC, заданная/текущая температура, вентилятор, вертикальный поток, связь и коды ошибок |

Один профиль охватывает MIM-B19N и MIM-B19NT, поскольку Samsung публикует для
них общую карту регистров. MIM является ведомым Modbus RTU (документированные
параметры по умолчанию: 9600 бод, 8E1) и связывается с оборудованием Samsung по
R1/R2. В форме подключения задаётся Modbus-адрес MIM, а на отдельном шаге —
адрес устройства Samsung (0–47). MIM отслеживает до 48 адресов; одна запись
Modbus Devices представляет один выбранный адрес.

Источник реализации — инструкция Samsung DB68-07538A-03. Каталожное описание и
фотография платы доступны на
[странице MIM-B19N](https://intech.cool/pl/akcesoria-do-pomp-ciepla-samusng/8111-interfejs-modbus-mim-b19n-do-pomp-ciepla-samsung.html).
...
| Модель | Подключение | Основные возможности |
|---|---|---|
| ER-G-220-05 | Прямой Modbus | Мониторинг и состояния насосного преобразователя; секундный опрос рабочих данных; команды пуска, останова и аварий; управление Y1/Y2; защищённая настройка параметров P001…P136 |
...
Состояние Home Assistant обновляется только после совпавшего readback.
Мониторинг рабочих данных, команды, выходы Y1/Y2 и декодирование версии ПО
подтверждены владельцем на физическом преобразователе.
