...
## Новое в 1.2.0 — мониторинг APC UPS и управление ТРМ-138

В релизе добавлен аппаратно проверенный мониторинг **APC Smart-UPS 3000 RM XL**
через Modbus TCP: состояния питания и батареи, электрические величины, нагрузка,
оставшееся время работы, температура, AVR и диагностика. Управление UPS не
предоставляется.

**ОВЕН ТРМ-138** теперь использует стабильные IEEE-754 значения всех восьми
температурных каналов и предоставляет документированные параметры
`C.dr 1` … `C.dr 8`. Они читаются через FC03 и записываются целыми значениями
от 0 до 8 через строго проверяемую команду FC06.
...
- Явно описанные модели оборудования APC, Bolid, Daikin, Dyna Drive, Haier, Owen и Zuked.
