...
## Новое в 1.3.0 — управление климатом 4VRS Haier-ESP32

В релизе добавлен отдельный профиль **4VRS Haier-ESP32** для проекта
[Haier-ESP32-Modbus](https://github.com/dk-1983/Haier-ESP32-Modbus): управление
климатом, тихим режимом, дисплеем и положением жалюзи, а также диагностика
команд и связи. Существующий профиль Haier YCJ-A002 не изменён.

Карта регистров использует заводской набор YCJ-A002 как совместимую основу и
дополняет его регистрами собственной расширенной функциональности проекта.

Записи физически сериализуются, ожидают подтверждения контроллера и проверяются
точным перечитыванием до обновления состояния Home Assistant. Базовый обмен и
работоспособность пути YCJ-A002 подтверждены на физическом стенде. Полная
проверка расширенных функций запланирована после изготовления печатной платы.
...
| TRM-138 | Прямой Modbus | Восемь температурных каналов IEEE-754, диагностика и настройка привязок выходов `C.dr 1…8` |
