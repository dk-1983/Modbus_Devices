"""Register the Lovelace device-card generator."""
...
FRONTEND_MODULE_FILENAME = "device-card-generator.js"
FRONTEND_MODULE_URL = f"{STATIC_URL}/{FRONTEND_MODULE_FILENAME}"
DEVICE_CARD_TAG = "modbus-device-card"
WS_BUILD_DEVICE_CARD = "modbus_devices/presentation/build"


def frontend_module_url(asset_dir: Path) -> str:
    """Return a content-versioned URL so an updated ES module executes again."""
    digest = sha256((asset_dir / FRONTEND_MODULE_FILENAME).read_bytes()).hexdigest()
    return f"{FRONTEND_MODULE_URL}?v={digest[:12]}"
...
async def async_register_dashboard_frontend(hass: HomeAssistant) -> None:
    """Register the generator module and its read-only builder command."""
    asset_dir = Path(__file__).parent / "frontend"
    await hass.http.async_register_static_paths(
        [StaticPathConfig(STATIC_URL, str(asset_dir), cache_headers=False)]
    )
    module_url = await hass.async_add_executor_job(frontend_module_url, asset_dir)
